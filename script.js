...
const loadingScreen=$("#loadingScreen"), loadingProgress=$("#loadingProgress"), loadingPercent=$("#loadingPercent");
let loadValue=0;
const loadingTimer=setInterval(()=>{ loadValue=Math.min(100,loadValue+4); loadingProgress.style.width=loadValue+"%"; loadingPercent.textContent=loadValue+"%"; if(loadValue>=100){clearInterval(loadingTimer);setTimeout(()=>loadingScreen.classList.add("hide"),350);} },80);
...
$("#openInvitation").addEventListener("click", async ()=>{
  tone("open");
  try{await audio.play();playing=true;$("#musicIcon").src="assets/MUSIK.png";}catch(e){showToast("Ketuk tombol musik jika browser menahan autoplay.");}
