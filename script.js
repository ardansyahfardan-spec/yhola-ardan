...
function finishTutorial(){
  ...
  setTimeout(showSketchIndicators, 120);
  tone("success");
}
$("#startExplore").addEventListener("click", finishTutorial);
$("#tutorialClose").addEventListener("click", finishTutorial);

function showSketchIndicators(){
  $$(".sketch-indicator").forEach(el=>el.remove());
  $$(".hotspot").forEach((hotspot, i)=>{
    const r=hotspot.getBoundingClientRect();
    const pad=10;
    const el=document.createElement("div");
    el.className="sketch-indicator";
    el.style.left=(r.left-pad)+"px";
    el.style.top=(r.top-pad)+"px";
    el.style.width=(r.width+pad*2)+"px";
    el.style.height=(r.height+pad*2)+"px";
    el.innerHTML='<svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><path d="M14,48 C8,20 29,8 51,13 C78,7 94,26 87,48 C96,72 74,91 50,85 C25,94 6,75 14,48 Z"></path></svg>';
