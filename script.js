...
function sfx(type='click'){unlockAudio();if(!audioCtx)return;const now=audioCtx.currentTime;if(type==='creak'){
  // Short, dry wooden crack: two irregular timber snaps with a low woody thump.
  const master=audioCtx.createGain();master.gain.setValueAtTime(.0001,now);master.gain.exponentialRampToValueAtTime(.5,now+.004);master.gain.exponentialRampToValueAtTime(.0001,now+.24);master.connect(audioCtx.destination);
  const makeCrack=(when,amp,dur)=>{const len=Math.floor(audioCtx.sampleRate*dur);const buf=audioCtx.createBuffer(1,len,audioCtx.sampleRate),data=buf.getChannelData(0);for(let i=0;i<len;i++){const t=i/len;data[i]=(Math.random()*2-1)*Math.pow(1-t,4)*amp}const src=audioCtx.createBufferSource(),filter=audioCtx.createBiquadFilter(),g=audioCtx.createGain();src.buffer=buf;filter.type='bandpass';filter.frequency.value=1800;filter.Q.value=1.1;g.gain.value=.75;src.connect(filter).connect(g).connect(master);src.start(when);src.stop(when+dur)};
  makeCrack(now,.95,.055);makeCrack(now+.075,.7,.045);
  const osc=audioCtx.createOscillator(),og=audioCtx.createGain();osc.type='triangle';osc.frequency.setValueAtTime(105,now);osc.frequency.exponentialRampToValueAtTime(48,now+.22);og.gain.setValueAtTime(.0001,now);og.gain.exponentialRampToValueAtTime(.32,now+.008);og.gain.exponentialRampToValueAtTime(.0001,now+.22);osc.connect(og).connect(master);osc.start(now);osc.stop(now+.23);return;
}
const gain=audioCtx.createGain(),osc=audioCtx.createOscillator();osc.connect(gain).connect(audioCtx.destination);const dur=.12,f=type==='start'?720:type==='lamp'?500:660;osc.type='triangle';osc.frequency.setValueAtTime(f,now);osc.frequency.exponentialRampToValueAtTime(420,now+dur);gain.gain.setValueAtTime(.0001,now);gain.gain.exponentialRampToValueAtTime(.2,now+.012);gain.gain.exponentialRampToValueAtTime(.0001,now+dur);osc.start(now);osc.stop(now+dur)}
window.addEventListener('pointerdown',unlockAudio,{once:true,passive:true});

/* Ambient butterflies + brighter roaming fireflies */
function initAmbient(){
  document.querySelectorAll('.ambient-layer').forEach((layer,idx)=>{
    ...
    for(let i=0;i<14;i++){
      ...
      f.className='firefly';
      f.style.left=(8+((i*17+idx*11)%84))+'%';
      f.style.top=(12+((i*23+idx*17)%76))+'%';
      f.style.setProperty('--fd',(3.8+(i%4)*1.15)+'s');
      f.style.setProperty('--fdelay',(-i*.72)+'s');
      f.style.setProperty('--fx',((i%5)*24-48)+'px');
      f.style.setProperty('--fy',(-14-(i%5)*10)+'px');
      layer.appendChild(f);
    }
    for(let i=0;i<2;i++){
      ...
      b.textContent='🦋';
      b.style.setProperty('--bd',(12+i*3)+'s');
      b.style.setProperty('--bdelay',(-(idx*2+i*5))+'s');
      b.style.setProperty('--bx',((i?1:-1)*18)+'px');
      layer.appendChild(b);
    }
  });
}
...
$('continueBtn').onclick=()=>{unlockAudio();sfx();$('tutorial').classList.add('hidden');$('scene').classList.remove('hidden');startExperience()};
/* music */
const m1=$('m1'),m2=$('m2');
function updateMusicIcon(){$('musicIcon').src=musicOn?'assets/MUSIC.png':'assets/MUTE.png'}
function startMusic(){musicOn=true;currentTrack=1;m1.volume=.78;m2.volume=.78;m2.pause();m1.currentTime=0;m1.play().catch(()=>{});m1.onended=()=>{if(musicOn){currentTrack=2;m2.currentTime=0;m2.play().catch(()=>{});}};m2.onended=()=>{if(musicOn){currentTrack=1;m1.currentTime=0;m1.play().catch(()=>{});}};updateMusicIcon()}
$('musicBtn').onclick=()=>{unlockAudio();sfx();const a=currentTrack===1?m1:m2;if(musicOn){a.pause();musicOn=false}else{musicOn=true;a.play().catch(()=>{});}updateMusicIcon()};
...
function addHot(id,rect){const [x1,y1,x2,y2]=rect,h=document.createElement('button');h.className='hot';h.setAttribute('aria-label',id);h.style.left=pct(x1,1080)+'%';h.style.top=pct(y1,1920)+'%';h.style.width=pct(x2-x1,1080)+'%';h.style.height=pct(y2-y1,1920)+'%';h.onclick=()=>{unlockAudio();sfx();h.classList.remove('bump');void h.offsetWidth;h.classList.add('bump');setTimeout(()=>openPopup(id),170)};$('hotspots').appendChild(h)}
objects.forEach(([id,file,glow,rect])=>{addLayer(id,file,glow);addHot(id,rect)});
/* chair */
function chairJolt(){const chair=document.querySelector('[data-id="chair"]');if(!chair)return;chair.animate([{transform:'translateY(0)'},{transform:'translateY(-6px) scale(1.008)'},{transform:'translateY(1px) scale(.997)'},{transform:'translateY(0)'}],{duration:520,easing:'cubic-bezier(.2,.8,.2,1)'});sfx('creak')}
function scheduleChair(){setTimeout(()=>{if(!document.hidden){chairJolt();chairJolt()}scheduleChair()},8000+Math.random()*5000)}
/* popup */
const popupData={
...
function closePopup(){const a=$('vnPlayer');if(a){a.pause();restoreMusic()}$('backdrop').classList.add('hidden');$('backdrop').setAttribute('aria-hidden','true')}$('close').onclick=()=>{sfx();closePopup()};$('backdrop').onclick=e=>{if(e.target===e.currentTarget)closePopup()};
/* info */
