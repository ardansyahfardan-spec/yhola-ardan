...
function startMusic(){musicOn=true;currentTrack=1;m1.volume=.78;m2.volume=.78;m1.pause();m2.pause();m1.currentTime=0;m2.currentTime=0;m1.onended=()=>{if(musicOn){currentTrack=2;m2.currentTime=0;m2.play().catch(()=>{});}};m2.onended=()=>{if(musicOn){currentTrack=1;m1.currentTime=0;m1.play().catch(()=>{});}};m1.play().catch(()=>{});updateMusicIcon()}
...
function addHot(id,rect){const [x1,y1,x2,y2]=rect,h=document.createElement('button');h.className='hot';h.setAttribute('aria-label',id);h.style.left=pct(x1,1080)+'%';h.style.top=pct(y1,1920)+'%';h.style.width=pct(x2-x1,1080)+'%';h.style.height=pct(y2-y1,1920)+'%';h.onclick=()=>{unlockAudio();sfx('object');h.blur();setTimeout(()=>openPopup(id),170)};h.onfocus=()=>h.blur();$('hotspots').appendChild(h)}
